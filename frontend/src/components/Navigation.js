...
const Navigation = () => {
  ...
  useEffect(() => {
    ...
    const fetchLiftRequestCount = async () => {
      try {
        ...
        for (const request of currentRequests) {
          if (!seenRequestIds.current.has(request.id) && request.requester_email !== user?.email) {
        ...
        // Update seen IDs
        seenRequestIds.current = new Set(currentRequests.map(r => r.id));
      } catch (error) {
  ...
  const handleAcceptLift = async (requestId) => {
    try {
      ...
      setNewRequest(null);
      // Refresh count
      const countRes = await liftRequestAPI.getCount();
      setLiftRequestCount(countRes.data.count);
    } catch (error) {
  ...
  const handleLiftRequestSuccess = async () => {
    const countRes = await liftRequestAPI.getCount();
    setLiftRequestCount(countRes.data.count);
  };
